...
export const MGMT_TASKS = [
  { n:'Review schedule & manage workshop flow', m:30   },
  ...
  { n:'Business coach meeting',                 m:null },
  ...
  { n:'R&D',                                    m:null },
  { n:'Build jigs',                             m:null },
  { n:'Improve processes',                      m:null },
  { n:'Stocktake & track materials',            m:null },
  { n:'Training',                               m:null },
  { n:'Performance reviews',                    m:null },
  { n:'Generate content',                       m:null },
];

export const WORKSHOP_TASKS = [
  { section:'Daily — all team',    n:'Set up workstation',                        m:15, role:'maker1'    },
  { section:'Daily — all team',    n:'Fill glue pots',                            m:10, role:'assistant' },
  { section:'Daily — all team',    n:'Clean kitchen',                             m:15, role:'assistant' },
  { section:'Daily — all team',    n:'Empty bins',                                m:10, role:'assistant' },
  { section:'Daily — MDF priming', n:'Prime MDF sheets (up to 3)',                m:60, role:'assistant' },
  { section:'Maintenance',         n:'Clean table saw & planer',                  m:30, role:'maker1'    },
  { section:'Maintenance',         n:'Clean down build station',                  m:30, role:'maker1'    },
  { section:'Maintenance',         n:'Replace vacuum bags',                       m:10, role:'assistant' },
...
export const PROD_TASKS = [
  ...
  { phase:'Drawer construction',        n:'Cut drawer components, sand edges, build & add runners', m:45, q:'draw', role:'maker1' },
  { phase:'Drawer construction',        n:'Fit drawer divide',                  m:20, q:'ddiv',    role:'maker1' },
  { phase:'Drawer construction',        n:'Fit drawer front, sand edges',       m:15, q:'dfront',  role:'maker1' },
  ...
  { phase:'Unit prep for painting',     n:'Remove runners',                     m:5,  q:'draw',    role:'painter' },
...
export function genClientTasks(cl) {
  ...
  for (const t of PROD_TASKS) {
    ...
    tasks.push({ id:`${cl.id}|${t.phase}|${t.n}`, n:qty>1?`${t.n} ×${qty}`:t.n, m:t.m*(t.q?qty:1), phase:t.phase, sugRole:t.role, assignedRole:null, done:false });
  }
...
export function genMgmtTasks() {
  return MGMT_TASKS.map(t=>({ id:`mgmt|${t.n}`, n:t.n, m:t.m, phase:'Management', sugRole:'manager', assignedRole:null, done:false, needsTime:t.m===null }));
}

export function genWsTasks() {
  return WORKSHOP_TASKS.map(t=>({ id:`ws|${t.n}`, n:t.n, m:t.m, phase:t.section, sugRole:t.role, assignedRole:null, done:false }));
}
